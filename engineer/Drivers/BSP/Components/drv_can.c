...
void can_manage_init(void)
{
  ...
  fifo_init(&(can1_manage.tx_fifo), can1_tx_fifo_buff, sizeof(struct can_std_msg),
  ...
  fifo_init(&(can2_manage.tx_fifo), can2_tx_fifo_buff, sizeof(struct can_std_msg),
...
uint32_t can_msg_bytes_send(CAN_HandleTypeDef *hcan, uint8_t *data, uint16_t len, uint16_t std_id)
{
  uint8_t *send_ptr;
  uint16_t send_num;
...
//    for (int i = 0; i < 5; i += 4)
//      *((uint32_t *) (msg.data + i)) = *((uint32_t *) (send_ptr + i));
