...
int main(void)
{
  ...
  Queue01Handle = osMessageQueueNew (4, sizeof(uint8_t*), &Queue01_attributes);

  /* definition and creation of Queue02 */
  const osMessageQueueAttr_t Queue02_attributes = {
    .name = "Queue02"
  };
  Queue02Handle = osMessageQueueNew (4, sizeof(uint8_t), &Queue02_attributes);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  UNUSED(huart);
  add=uart_buff;
  xQueueSendFromISR(Queue01Handle,&add,0);
  HAL_UART_Receive_IT(&huart1,uart_buff,1);
...
void StartDefaultTask(void *argument)
{
  ...
  for(;;)
  {
     ...
     if(send>=9)send=0;
//     add=uart_buff;
//     xQueueSend(Queue01Handle,&add,0);
  //   xQueueSend(Queue02Handle,&send,0);
    osDelay(5);
...
void StartTask02(void *argument)
{
 ...
 //     xQueueReceive(QUEUE1,rx_buff1,0);
      if(xQueueReceive(Queue01Handle,&rx_buff1,0)==pdTRUE)//receive时间100时会卡死
	  {
	  message[0]=*rx_buff1;
